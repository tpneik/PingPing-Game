...
def connect():
    try:
        ...
        conn.connect_ex((settings.SERVER_IP, settings.SERVER_PORT))
        me = str(conn.getsockname()[1])
        print(f"Client connected to {settings.SERVER_IP}:{settings.SERVER_PORT} with id: {me}")
        return [me, conn]
    except socket.error:
        print(f"Couldn't connect to game server in: {settings.SERVER_IP}:{settings.SERVER_PORT}")
        sys.exit(1)
...
class Game(pyglet.window.Window):
    ...
    def __init__(self, multiplayer_mode = False):
        self.load_sprites()
        self.sound_racket = pyglet.resource.media(settings.HIT_RACKET_SOUND, streaming=False)
        self.sound_wall = pyglet.resource.media(settings.HIT_WALL_SOUND, streaming=False)
    ...
    def load_sprites(self):
        ...
        self.winner_text = pyglet.text.Label('YOU\'RE WINNING', font_size=20, x=settings.WINDOW_WIDTH/2, y=settings.WINDOW_HEIGHT/2, anchor_x='center', anchor_y='center', color=(128, 0, 255, 128))
        self.loser_text = pyglet.text.Label('YOU\'RE LOSING', font_size=20, x=settings.WINDOW_WIDTH/2, y=settings.WINDOW_HEIGHT/2, anchor_x='center', anchor_y='center', color=(128, 0, 255, 128))
    ...
    def on_collision(self):
        ...
        if player:
            ...
            self.sound_racket.play()
        if self.ball.check_collision_laterals(settings.WINDOW_HEIGHT):
            self.ball.hit_lateral()
            self.sound_wall.play()
        if self.ball.check_collision_left(self.racket_left):
            self.ball.hit_side()
            self.racket_right.increase_score()
            self.sound_wall.play()
        if self.ball.check_collision_right(settings.WINDOW_WIDTH):
            self.ball.hit_side()
            self.racket_left.increase_score()
            self.sound_wall.play()
    ...
    def reset_score(self):
        self.racket_left.SCORE = 0
        self.racket_right.SCORE = 0

    def win_or_lose(self):
        me = self.racket_me.SCORE
        enemy = self.racket_vs.SCORE
        if self.running:
            if (me > enemy) and (me - enemy == settings.SCORE_DISTANCE):
                return True
            if (enemy > me) and (enemy - me == settings.SCORE_DISTANCE):
                return False
        return None
