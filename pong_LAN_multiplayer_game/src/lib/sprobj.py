...
class SprObj(pyglet.sprite.Sprite):
    ...
    def check_collision(self, others_list):
        for sprite in others_list:
            if (self.bottom <= sprite.top and self.top >= sprite.bottom and self.right >= sprite.left and self.left <= sprite.right):
                print(f"Object collision: {self.position} with {sprite.position}")
                return sprite

    def check_collision_laterals(self, window_height):
        if self.top > window_height or self.bottom < self.height :
            print(f"Lateral collision: {self.position} with window")
            return True


    def check_collision_left(self,racket):
        if self.left < racket.x:
            print(f"Lateral collision: {self.position} with window")
            return True

    def check_collision_right(self,window_width):
